...
module = ai_train.AiModule()

model = module.get_model()
model.load_weights('./training/weights')
...
while game.is_over() == False:
    ...
    if game.whose_turn() == 1:
        best_one = possible_moves[np.argmax(np.array([model.predict(ai_train.create_numerical_board(game.board.create_new_board_from_move(move)))[0][0] for move in possible_moves]))]
        curr_player = game.whose_turn()
        game.move(best_one)
        if ai_train.check_if_move_is_continous(previous_moves[curr_player - 1], best_one) == True:
            break
        ...
        if ai_train.check_if_move_is_continous(previous_moves[curr_player - 1], possible_moves[move]) == True:
            break
        previous_moves[curr_player - 1] = previous_moves[curr_player - 1][1:] + [best_one]
    ai_train.print_board(ai_train.create_numerical_board_to_print(game.board))"""
